...
**Error codes**: `VALIDATION_FAILED` (400), `INVALID_JSON` (400), `BAD_REQUEST` (400), `PAYLOAD_TOO_LARGE` (413), `UNSUPPORTED_MEDIA_TYPE` (415), `RATE_LIMITED` (429), `ADDRESS_UNAVAILABLE` (429), `GLOBAL_LIMIT_REACHED` (429), `INSUFFICIENT_BALANCE` (503), `SERVICE_BUSY` (503), `REQUEST_EXPIRED` (503), `SERVICE_UNAVAILABLE` (503), `FUSE_FAILED` (500), `INTERNAL_ERROR` (500). All errors include `success: false` and a structured `error` object with `code` and `message`.
